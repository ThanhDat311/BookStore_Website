            ...
            <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                <table class="table table-bordered" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                        <?php $max = min(5, count($products)); for ($i = 0; $i < $max; $i++): $p = $products[$i]; ?>
                        <tr>
                            <td><?= $p['id'] ?></td>
                            <td><?= htmlspecialchars($p['name']) ?></td>
                            <td><?= number_format($p['price'], 0, ',', '.') ?>₫</td>
                            <td><?= htmlspecialchars($p['category_name']) ?></td>
                            <td><?php if ($p['thumbnail']): ?><img
                                    src="../../assets/images/product-images/<?= htmlspecialchars($p['thumbnail']) ?>"
                                    width="60"><?php endif; ?></td>
                            <td><?= $p['stock'] ?></td>
                            <td><?= $p['created_at'] ?></td>
                            <td>
